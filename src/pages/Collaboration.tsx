...
const Collaboration = () => {
  ...
  const handleConfirmAcceptRejectAgreement = async () => {
    ...
    try {
      const requesterProfile = requesterProfiles[selectedRequest.requester_id];
      if (requesterProfile?.email) {
        const requesterName = `${requesterProfile.first_name || ''} ${requesterProfile.last_name || ''}`.trim() || 'there';
        const myName = `${profile?.first_name || ''} ${profile?.last_name || ''}`.trim() || 'A researcher';
        
        await supabase.functions.invoke('send-collaboration-email', {
          body: {
            type: acceptRejectAction === 'accept' ? 'accepted' : 'rejected',
            recipientEmail: requesterProfile.email,
            recipientName: requesterName,
            senderName: myName,
            senderInstitution: profile?.institution,
            projectDescription: selectedRequest.start_date && selectedRequest.end_date 
              ? `Collaboration period: ${selectedRequest.start_date} to ${selectedRequest.end_date}`
              : undefined
          }
        });
        console.log('Email notification sent to requester');
      }
    } catch (emailError) {
      console.error('Failed to send email notification:', emailError);
      // Don't show error to user as the action was successful
    }
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="acceptRejectTermsAccepted" className="text-sm text-gray-700 cursor-pointer">
                      I agree to the terms and conditions
                    </label>
                  </div>
                  
                  {/* Terms Content - Shows when checkbox is checked */}
                  {acceptRejectTermsAccepted && (
