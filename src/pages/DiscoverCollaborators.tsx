...
const DiscoverCollaborators = () => {
  ...
  const getMatchingCollaborators = () => {
    if ((!profile?.what_i_have || profile.what_i_have.length === 0) && 
      ...
      // If user has no items in either field, show collaborators who also haven't filled those fields
      return collaborators.filter(collaborator => 
        (!collaborator.what_i_have || collaborator.what_i_have.length === 0) || 
        (!collaborator.what_i_need || collaborator.what_i_need.length === 0)
      );
    }
    ...
    // If no matches found, return 7th, 9th, and 17th collaborators as fallback
    if (matchingCollaborators.length === 0) {
      const fallbackIndices = [6, 8, 16]; // 7th, 9th, 17th collaborators (0-based indexing)
      return fallbackIndices
        .filter(index => index < collaborators.length)
        .map(index => collaborators[index]);
    }
  ...
  const handleConfirmAgreement = async () => {
    ...
    // Check if there's an existing request first
    ...
    if (existingRequest) {
      ...
      // Insert new request
      ({ error } = await supabase.from('collaborations').insert([payload as any]));
    }
    ...
    if (error) {
      ...
      try {
        const collaborator = selectedProfile as any;
        const collaboratorName = `${collaborator.first_name || ''} ${collaborator.last_name || ''}`.trim() || 'there';
        const requesterName = `${profile?.first_name || ''} ${profile?.last_name || ''}`.trim() || 'A researcher';
        ...
        await supabase.functions.invoke('send-collaboration-email', {
          body: {
            ...
            senderInstitution: profile?.institution,
            projectDescription: `Collaboration request for the period ${startDate.toISOString().slice(0, 10)} to ${endDate.toISOString().slice(0, 10)}`
          }
        });
        console.log('Email notification sent successfully');
      } catch (emailError) {
        console.error('Failed to send email notification:', emailError);
        // Don't show error to user as the collaboration request was successful
      }
                  ...
                  <div className="pt-4 border-t space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center space-x-2">
                        ...
                        <label htmlFor="termsAccepted" className="text-sm text-gray-700 cursor-pointer">I agree to the terms and conditions</label>
                      </div>
                      
                      {/* Terms Content - Shows when checkbox is checked */}
                      {termsAccepted && (
                        <div className="mt-3 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                          <h4 className="text-sm font-medium text-gray-900 mb-2">Terms of Service</h4>
                          <div className="text-xs text-gray-700 leading-relaxed space-y-2">
                            <p>
                              <strong>Collaboration Agreement:</strong> I agree to work in good faith, sharing expertise and responsibilities to achieve the objectives of the collaboration. I commit to maintaining integrity, transparency, and confidentiality where required.
                            </p>
                            <p>
                              <strong>Ethical Standards:</strong> I adhere to general ethical standards of collaboration and make fair contributions to the collaborative projects. All work must comply with academic integrity and research ethics guidelines.
                            </p>
                            <p>
                              <strong>Dispute Resolution:</strong> In case of disagreements, issues should be resolved through open, civilized and trustful dialogue. Both parties commit to professional communication and mutual respect.
                            </p>
                            <p>
                              <strong>Data and IP Rights:</strong> All parties retain rights to their original contributions while agreeing to share collaborative outputs as mutually determined. Proper attribution will be given to all contributors.
                            </p>
                            <p>
                              <strong>Termination:</strong> Either party may terminate the collaboration with reasonable notice, ensuring proper handover of shared responsibilities and materials.
                            </p>
                          </div>
                        </div>
                      )}
