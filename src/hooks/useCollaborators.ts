...
export const useCollaborators = () => {
  ...
  // Check if a collaborator is best match - now handled client-side
  const isBestMatch = (collaboratorId: string) => {
    return false; // Always return false since matching is handled in UI
  };
  ...
  const initializeRandomData = async () => {
    ...
    try {
      ...
      if (collaborators.length > 0) {
        ...
        // best_match column removed - matching is now handled client-side
